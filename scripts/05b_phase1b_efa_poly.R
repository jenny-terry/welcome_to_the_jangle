...
p1b_poly_efa_data <- dplyr::select(phase1_data, q4.1_1:q4.4_19)
...
#check missing
p1b_poly_efa_data %>% finalfit::ff_glimpse()

# descriptive statistics (this section hates piping, so meh)
p1b_poly_efa_descriptives <- psych::describe(p1b_poly_efa_data) 
p1b_poly_efa_descriptives$item <- row.names(p1b_poly_efa_descriptives)  
p1b_poly_efa_descriptives <- tibble::as_tibble(p1b_poly_efa_descriptives) 
p1b_poly_efa_descriptives <- p1b_poly_efa_descriptives |>
  dplyr::select(item, mean, sd, min, max, skew, kurtosis, se)

# correlation matrix
p1b_poly_efa_data <- p1b_poly_efa_data |> 
  dplyr::select(order(colnames(p1b_poly_efa_data)))

p1b_poly_efa_corr <- lavaan::lavCor(object = p1b_poly_efa_data, 
                                    ordered = names(p1b_poly_efa_data),
                                    output = "cor")

p1b_poly_efa_corr <- as.data.frame(p1b_poly_efa_corr)

# high & low corrs
p1b_poly_efa_corr %<>% tibble::as_tibble() 
p1b_poly_high_corrs <- sum(p1b_poly_efa_corr >= 0.9)  
p1b_poly_high_corrs70 <- sum(p1b_poly_efa_corr >= 0.7)  
p1b_poly_low_corrs <- sum(p1b_poly_efa_corr < 0.2)
p1b_poly_no_corrs <- nrow(p1b_poly_efa_corr)*ncol(p1b_poly_efa_corr)
p1b_poly_high_corrs_percent <- p1b_poly_high_corrs/p1b_poly_no_corrs*100
p1b_poly_high_corrs70_percent <- p1b_poly_high_corrs70/p1b_poly_no_corrs*100
p1b_poly_low_corrs_percent <- p1b_poly_low_corrs/p1b_poly_no_corrs*100

# KMO test
p1b_poly_efa_kmo <- psych::KMO(p1b_poly_efa_data)

# determinant
p1b_poly_efa_det <- det(as.matrix(p1b_poly_efa_corr))

#bartlett
p1b_poly_efa_bart <- psych::cortest.bartlett(p1b_poly_efa_corr, n = 489)

# test for the number of factors
p1b_poly_efa_out1 <- psych::fa.parallel(p1b_poly_efa_data, fa = "fa", fm = "ml", cor = "poly", SMC = T, n.iter = 100)
p1b_poly_efa_out2 <- psych::vss(p1b_poly_efa_data, fm = "ml", cor = "poly")

# create indicator table
Indicator <- c("Parallel Analysis", "VSS Complexity 1", "VSS Complexity 2", "Velicer MAP", "BIC", "SABIC", "RMSEA", "SRMR")
Factors <- c(p1b_poly_efa_out1$nfact, 1, 2, 8, 8, 8, 1, 8)

p1b_efa_nfact_table <- tibble::tibble(Indicator, Factors) |> 
  dplyr::arrange(desc(Factors))

# rotate solution
p1b_poly_efa_mod8 <- psych::fa(p1b_poly_efa_data, nfactors = 8, rotate = "oblimin", fm = "ml", cor = "poly", SMC = T, p = .0)

p1b_poly_efa_mod15 <- psych::fa(p1b_poly_efa_data, nfactors = 15, rotate = "oblimin", fm = "ml", cor = "poly", SMC = T, p = .0)
...
p1b_poly_efa_loadings8 <- printLoadings(p1b_poly_efa_mod8$loadings, digits = 2, cutoff = 0, sort = T)
p1b_poly_efa_loadings15 <- printLoadings(p1b_poly_efa_mod15$loadings, digits = 2, cutoff = 0, sort = T)

p1b_poly_efa_loadings8 <- tibble::rownames_to_column(as.data.frame(p1b_poly_efa_loadings8), "items")
p1b_poly_efa_loadings15 <- tibble::rownames_to_column(as.data.frame(p1b_poly_efa_loadings15), "items")