...
p1a_efa_data <- dplyr::select(phase1_data, c(q4.1_1:q4.1_23, q4.3_1:q4.3_20))
...
#check missing
p1a_efa_data %>% finalfit::ff_glimpse()

# descriptive statistics (this section hates piping, so meh)
p1a_efa_descriptives <- psych::describe(p1a_efa_data) 
p1a_efa_descriptives <- tibble::as_tibble(p1a_efa_descriptives) 
p1a_efa_descriptives <- p1a_efa_descriptives |>
  dplyr::select(mean, sd, min, max, skew, kurtosis, se)

# covariance matrix using fiml
p1a_efa_cov <- psych::corFiml(p1a_efa_data)

# descriptive correlations 
p1a_efa_corr <- cov2cor(p1a_efa_cov)

# high & low corrs
p1a_efa_corr %<>% tibble::as_tibble() 
p1a_high_corrs <- sum(p1a_efa_corr >= 0.9)  
p1a_high_corrs70 <- sum(p1a_efa_corr >= 0.7)  
p1a_low_corrs <- sum(p1a_efa_corr < 0.2)
p1a_no_corrs <- nrow(p1a_efa_corr)*ncol(p1a_efa_corr)
p1a_high_corrs_percent <- p1a_high_corrs/p1a_no_corrs*100
p1a_high_corrs70_percent <- p1a_high_corrs70/p1a_no_corrs*100
p1a_low_corrs_percent <- p1a_low_corrs/p1a_no_corrs*100

# KMO test
p1a_efa_kmo <- psych::KMO(p1a_efa_data)

# determinant
p1a_efa_det <- det(as.matrix(p1a_efa_corr))

# test for the number of factors 
p1a_efa_out1 <- psych::fa.parallel(p1a_efa_cov, n.obs = 489, fa = "fa", fm = "pa", SMC = T, n.iter = 100)
p1a_efa_out2 <- psych::vss(p1a_efa_cov, n.obs = 489, fm = "pa")
...
obs <- data.frame(p1a_efa_out1$fa.values)
...
percentile <- apply(p1a_efa_out1$values, 2, function(x) quantile(x,.95))
...
library(ggplot2)
p1a_scree_plot <- ggplot(eigendat, aes(x = num, y = eigenvalue, shape = type)) +
  geom_line() +
  geom_point(size = 4) +
  scale_y_continuous(name = 'Eigenvalue') +
  scale_x_continuous(name = 'Factor Number', breaks = min(eigendat$num):max(eigendat$num)) +
  scale_shape_manual(values = c(16, 1)) +
  geom_vline(xintercept = p1a_efa_out1$nfact, linetype = 'dashed') +
  jtools::theme_apa()
p1a_scree_plot
...
Factors <- c(p1a_efa_out1$nfact, 6, 1, 2, 5, 5, 8, 8, 8)

p1a_efa_nfact_table <- tibble::tibble(Indicator, Factors) |> 
  dplyr::arrange(desc(Factors))

# rotate solution
p1a_efa_mod6 <- psych::fa(p1a_efa_cov, n.obs = 489, nfactors = 6, fm = "pa", SMC = T, n.iter = 1, p = .05)
...
p1a_efa_loadings6 <- printLoadings(p1a_efa_mod6$loadings, digits = 2, cutoff = 0, sort = T)
p1a_efa_loadings6 <- tibble::rownames_to_column(as.data.frame(p1a_efa_loadings6), "items")
